...
-- Set our defaults
default("pets.deckprice", 2500000)
default("pets.craft.rogue", 1)
default("pets.craft.sword", 1)
default("pets.craft.mage", 1)
default("pets.craft.demon", 1)
default("pets.craft.vanilla", 1)
default("pets.craft.tbc", 1)
default("pets.craft.wotlk", 1)
default("pets.overhead.primal", 100000)
default("pets.overhead.eternal", 150000)
default("pets.mats.primal", 1)
default("pets.mats.eternal", 1)
...
function lib:MakeGuiConfig(gui)
	...
	gui:AddControl(id, "MoneyFramePinned",  0, 0, "pets.deckprice", 1, 99999999, "Budget per Epic Deck / 350 rep");
	gui:GetLast(id).clearance = 20;
	gui:AddControl(id, "MoneyFramePinned",  0, 0, "pets.overhead.primal", 1, 99999999, "Budget per Primal Life");
	gui:AddControl(id, "MoneyFramePinned",  0, 0, "pets.overhead.eternal", 1, 99999999, "Budget per Eternal Life");

	gui:SetLast(id, last)

	gui:AddControl(id, "Checkbox",          0.4, 1, "pets.craft.rogue"  , "Craft Rogues cards (85 Inscription)"      ); -- lvl 10
	gui:AddControl(id, "Checkbox",          0.4, 1, "pets.craft.sword"  , "Craft Swords cards (125 Inscription)"     ); -- lvl 10
	gui:AddControl(id, "Checkbox",          0.4, 1, "pets.craft.mage"   , "Craft Mages cards (175 Inscription)"      ); -- lvl 20
	gui:AddControl(id, "Checkbox",          0.4, 1, "pets.craft.demon"  , "Craft Demons cards (225 Inscription)"     ); -- lvl 20
	gui:AddControl(id, "Checkbox",          0.4, 1, "pets.craft.vanilla", "Craft Vanilla WoW cards (275 Inscription)"); -- lvl 35

	gui:AddControl(id, "Checkbox",          0.4, 1, "pets.craft.tbc"    , "Craft TBC cards (325 Inscription)"        ); -- lvl 50
	gui:AddControl(id, "Checkbox",          0.4, 2, "pets.mats.primal" , "Include Primal Lifes");
	gui:GetLast(id).clearance = 5;

	gui:AddControl(id, "Checkbox",          0.4, 1, "pets.craft.wotlk"  , "Craft WotLK cards (400 Inscription)"      ); -- lvl 65
	gui:AddControl(id, "Checkbox",          0.4, 2, "pets.mats.eternal", "Include Eternal Lifes");
	gui:GetLast(id).clearance = 5;
end
...
function lib.Search(item)
	...
	local limit = get("pets.deckprice");
	...
	--
	-- heavy parchment: 0.12.50
	if (get("pets.craft.vanilla")) then
	...
	if (get("pets.craft.tbc")) then
		if (tbc50Herbs[itemid]) then per_deck = 8 * 10 * 3; overhead = 8 * (5000 + (3 * get("pets.overhead.primal"))); end
		if (tbc25Herbs[itemid]) then per_deck = 8 * 20 * 3; overhead = 8 * (5000 + (3 * get("pets.overhead.primal"))); end
		if (tbcInks[itemid]   ) then per_deck = 8 * 3;      overhead = 8 * (5000 + (3 * get("pets.overhead.primal"))); end
	end
	...
	--
	-- resilient parchment: 0.50.00
	if (get("pets.craft.wotlk")) then
		if (wotlk50Herbs[itemid] ) then per_deck = 8 * 10 * 6 * 2; overhead = 8 * (5000 + (3 * get("pets.overhead.eternal"))); end
		if (wotlk25Herbs[itemid] ) then per_deck = 8 * 20 * 6 * 2; overhead = 8 * (5000 + (3 * get("pets.overhead.eternal"))); end
		if (wotlkPigments[itemid]) then per_deck = 8 * 6 * 2;      overhead = 8 * (5000 + (3 * get("pets.overhead.eternal"))); end
		if (wotlkInks[itemid]    ) then per_deck = 8 * 6;          overhead = 8 * (5000 + (3 * get("pets.overhead.eternal"))); end
	end
	...
	if (get("pets.mats.primal")) then
		local p1 = get("pets.overhead.primal");
	...
	if (get("pets.mats.eternal")) then
		local p2 = get("pets.overhead.eternal");
